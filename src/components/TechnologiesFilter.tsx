    ...
    <>
      <div className="flex flex-wrap justify-center gap-2 shrink-1">
        ...
        <button
          ...
          className={`${active === 'learning' ? 'bg-adriPink text-white' : 'bg-gray-200 text-black'}  py-2 px-5 rounded-lg flex items-center justify-evenly gap-1`}>
          Estilos
        </button>
      ...
      <div className="flex flex-row flex-wrap items-center justify-center w-2/3 mt-6 gap-5">
        {filteredTechnologies.map((tech: TechnologyType) => (
          <div key={tech.id} className="relative flex items-center justify-center w-48 p-4 px-1 py-2 overflow-hidden bg-blue-400 rounded-2xl bg-opacity-5 hover:border  hover:bg-opacity-10 h-14">
            <p className='absolute text-xl bottom-1 left-3 max-w-10 leading-5'> {tech.name}</p>
            <Image src={`/tecnology-icons/${tech.icon}`} width={40} height={40}
